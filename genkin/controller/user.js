...
module.exports = {
  ...
  signup: (req, res) => {
    User.findOne({ email: req.body.email }, (error, data) => {
      if (error) {
        //check for any error that would make data unavilable
        console.log("something went wrong");
        return res.status(400).json(ERR("something went wrong"));
      } //else if no error
      else {
        if (data && Object.keys(data).length > 0) {
          //check if data alrady exits in the collection
          return res
            .status(500)
          ...
          //else if the data doesn't exists in the collection
              ...
              Token.create(
                ...
                (error, tokken) => {
                  if (error) {
                    ...
                    return res.status(500).json(ERR("Error in token creation"));
                  }
  ...
  verificationtoken: (req, res) => {
    Token.findOne({ token: req.body.token }, function (error, token) {
      if (error) {
        //error occurred or token expired
        return res
          .status(400)
      ...
      if (token) {
          ...
          (error, data) => {
            if (error) {
              return res
                .status(400)
                .json(
                  ERR("Error ecountered while checking for token verification.")
                );
            }
            if (!data) {
              return res
        ...
        return res
          ...
          .json(
            ERR(
              "Unable to find a valid token or Token expired. Try Signing up again"
            )
          );
      }
    });
        ...
        return res
          ...
          .json(
            ERR(
              "This email address " +
                req.body.email +
                " is not associated with any account. Re-type your email address and try again."
            )
          );
      } else {
            ...
            PasswordToken.create(
              ...
              (error, tokken) => {
                ...
                if (tokken) {
                  ...
                  sgMail.send(mail, (error) => {
                    if (error) {
                      return res.status(401).json(ERR("Mail sending failed"));
                    }
                    return res
                      .status(200)
                      .json(
                ...
                if (user) {
                  ...
                  user.save((error) => {
                    ...
                    return res
                      .status(200)
                      .json(
                        SUCCESS("Password Changed, Shop with new password.")
                      );
                  });
                }
              }
  ...
  productcart: (req, res) => {
    ...
    Store.aggregate(aggregate, (err, data) => {
      if (err) {
  ...
  getAllproduct: (req, res) => {
    Products.find({}, (error, pdata) => {
      ...
      return res.status(200).json(SUCCESS(pdata));
    }).sort({ uploadDate: -1 });
  },
