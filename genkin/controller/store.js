...
module.exports = {
  create: (req, res) => {
    ...
    Owner.findOne({ _id: payload.owner }, (error, staff) => {
      ...
      if (!staff) {
        return res.status(502).json(ERR("Staff does not exist"));
      } else {
        Store.findOne(
          { $or: [{ _ownerId: staff._id }, { workers: staff._id }] },
          (error, store) => {
            ...
            if (error) {
              return res
                ...
                .json(
                  ERR(
                    "Error encountered while checking if you owned a store already"
                  )
                );
            }
            if (store) {
              ...
              Store.findOne({ name: req.body.name }, (error, storee) => {
                ...
                if (storee) {
                  return res
                    .status(404)
                    .json(
                      ERR("Store name already in use, use something unique")
                    );
                } else {
                  ...
                  Store.create(
                    ...
                    (error, store) => {
                      ...
                      if (!store) {
                        return res.status(404).json(ERR("No store created."));
                      }
                      if (store) {
                        staff.isAdmin = true;
                        ...
                        staff.save((error) => {
                          if (error) {
                            return res
                              ...
                              .json(
                                ERR(
                                  "Error while trying  to upgrade worker to Admin"
                                )
                              );
                          }
                          return res
                            .status(200)
                            .json(
                              SUCCESS("You are now the Admin for " + store.name)
                            );
                        });
                      }
                    }
  ...
  joinstore: (req, res) => {
    ...
    Owner.findOne({ _id: payload.owner }, (error, staff) => {
      ...
      if (staff.isAdmin === true) {
        return res.status(502).json(ERR("You already own your own store"));
      } else {
        Store.findOne(
          { $or: [{ _ownerId: staff._id }, { workers: staff._id }] },
          (error, person) => {
            ...
            if (person) {
              return res.status(401).json(ERR("You already belong to a store"));
            } else {
              Store.findOne({ hirekey: req.body.hirekey }, (error, key) => {
                ...
                if (key) {
                  Store.updateOne(
                    { hirekey: req.body.hirekey },
                    {
                      $addToSet: { workers: staff._id },
                      $pull: { hirekey: req.body.hirekey },
                    },
                    (error, update) => {
                      if (error) {
                        return res
                        ...
                        return res
                          ...
                          .json(
                            SUCCESS(
                              "You just joined " + key.name + " as a worker"
                            )
                          );
                      }
                    }
                  ...
                  return res.status(402).json(ERR("Incorrect key!"));
                }
              });
            }
          }
  ...
  removeproduct: (req, res) => {
    ...
    Owner.findOne({ _id: payload.owner }, (error, staff) => {
      if (error) {
        return res.status(400).json(ERR("Error fetching identity."));
      }
      if (staff.isAdmin === true) {
        Store.findOne({ _ownerId: staff._id }, (error, store_admin) => {
          ...
          if (store_admin) {
            Product.findOneAndRemove(
              { $and: [{ name: req.body.name }, { store: store_admin._id }] },
              (error, product) => {
                if (error) {
                  return res
                    .status(400)
                    .json(
                      ERR("Error encountered while trying to remove product.")
                    );
                }
                if (product) {
                  return res
                    ...
                    .json(
                      SUCCESS(
                        product.name + " removed from " + store_admin.name
                      )
                    );
                } else {
            ...
            return res
              ...
              .json(
                ERR(
                  "You are not the Admin for the store you're trying to access."
                )
              );
          }
        });
  ...
  removeworker: (req, res) => {
    let payload = req.decoded;
    Owner.findOne({ _id: payload.owner }, (error, owner) => {
      //find signed in entity
      if (error) {
        return res
          .status(400)
      ...
      if (owner.isAdmin === true) {
        //is the signed in entity an admin whatsoever?
        Owner.findOne({ username: req.body.username }, (error, staff) => {
          //find the guy we're removing
          if (error) {
            return res
              .status(400)
              .json(ERR("Error encountered while fetching Identity."));
          ...
          if (staff.isAdmin === false) {
            Store.update(
              { _ownerId: owner._id }, //signed in entity is store owner
              ...
              (error, unstaffed) => {
                if (error) {
                  return res
                    .status(400)
                    .json(
                      ERR("Error encountered while trying to remove worker")
                    );
                }
                if (unstaffed) {
                  ...
                  var mail = {
                    ...
                    text:
                      ...
                      staff.username +
                      "\n\n" +
                      "This is to officially inform you that you have been officially relieved of your official duties at " +
                      unstaffed.name +
                      "." +
                      "\n\n" +
                      "Thanks, Management.",
                  ...
                  sgMail.send(mail, (error) => {
                    ...
                    return res
                      ...
                      .json(
                        SUCCESS(
                          staff.username +
                            " has been relieved of their official duties and staff relieve mail has been sent to them, " +
                            staff.email
                        )
                      );
                  });
                  ...
                  return res
                    ...
                    .json(
                      ERR(
                        staff.username +
                          " never worked for you, so you can't remove them."
                      )
                    );
                }
              }
            ...
            return res
              .status(400)
              .json(
                ERR("Store owners can't be kicked out of their own store.")
              );
          }
        });
      } else {
